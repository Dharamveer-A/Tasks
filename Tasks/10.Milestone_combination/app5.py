...
if jd_file and resume_file:
    ...
    st.success("✅ Milestone 1 Completed: Documents uploaded, parsed, cleaned and previewed successfully.")
    ...
    def plot_category_match_comparison(resume_skills, jd_skills):
        """Plot categorized skill match comparison as bar chart"""
        jd_categorized = categorize_skills(jd_skills)
        resume_categorized = categorize_skills(resume_skills)
        
        all_categories = set(jd_categorized.keys()) | set(resume_categorized.keys())
        
        if not all_categories:
            st.warning("No skill categories found")
            return None
        ...
        for category in sorted(all_categories):
            jd_cat_skills = set(jd_categorized.get(category, []))
            resume_cat_skills = set(resume_categorized.get(category, []))
            
            if len(jd_cat_skills) > 0:
                categories.append(category)
                jd_counts.append(len(jd_cat_skills))
                matched = len(jd_cat_skills & resume_cat_skills)
                resume_counts.append(matched)
                match_percentages.append((matched / len(jd_cat_skills)) * 100)
        ...
        fig.add_trace(go.Bar(
            ...
            y=resume_counts,
            marker_color='#4a5bdc',
            text=[f'{count} ({pct:.0f}%)' for count, pct in zip(resume_counts, match_percentages)],
            textposition='outside'
        ))
        
        fig.update_layout(
            title="Category-wise Skill Match Analysis",
            barmode="group",
            height=500,
    ...
    st.markdown("""
        <div style="background-color:#4a5bdc;padding:20px;border-radius:10px;margin-top:30px;">
            <h2 style="color:white;">Milestone 3: Skill Gap Analysis and Similarity Matching Module</h2>
    ...
    if resume_all_skills and jd_all_skills:
        ...
        st.subheader("📊 Category-wise Skill Gap Analysis")
        fig_category = plot_category_match_comparison(resume_all_skills, jd_all_skills)
        if fig_category:
    ...
    <h2 style="color:white;text-align:center;">Milestone 4: Dashboard and Report Export Module</h2>
    ...
    if resume_all_skills and jd_all_skills and len(similarity_matrix) > 0:
        ...
        area_df = pd.DataFrame({'Skill': jd_all_skills, 'Similarity Score (%)': avg_similarity_scores})
        area_fig = go.Figure()
        area_fig.add_trace(go.Scatter(x=area_df['Skill'], y=area_df['Similarity Score (%)'], fill='tozeroy', name='Similarity Score', line_color='#7c3aed', fillcolor='rgba(124, 58, 237, 0.3)'))
        area_fig.update_layout(title="Average Similarity Score per Job Requirement", xaxis_title="Skills", yaxis_title="Similarity Score (%)", height=400, hovermode='x unified', yaxis=dict(range=[0, 100]))
