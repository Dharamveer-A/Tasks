...
def extract_text(file):
    ...
    try:
            ...
            with pdfplumber.open(file) as pdf:
                ...
                for page_num, page in enumerate(pdf.pages, 1):
                    ...
                    if page_text:
                        ...
                        st.warning(f"⚠️ Page {page_num} appears to be empty or contains only images")
                
                if not text.strip():
                    st.error("No text could be extracted from the PDF. The file might contain only images.")
                    return ""
        ...
        st.info("💡 Try re-uploading the file or use a different format (PDF/DOCX/TXT)")
        return ""
...
# ---------------- ANALYZE BUTTON ----------------
analyze_button = st.button("🔍 Analyze Documents", type="primary", use_container_width=True)
...
if analyze_button and jd_file and resume_file:
    ...
    def plot_category_match_heatmap(resume_skills, jd_skills):
        ...
        if not common_categories:
            st.warning("⚠️ No common skill categories found between resume and JD")
            return None
    ...
    if resume_all_skills and jd_all_skills and len(similarity_matrix) > 0:
        ...
        if category_similarities:
            ...
            st.info("ℹ️ No category data available for similarity analysis")
    else:
        st.info("ℹ️ Upload documents to see similarity score distribution")
    ...
    with col2:
        ...
        st.download_button("⬇️ Download DOCX Report", doc_file, "skill_gap_analysis_report.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", help="Download formatted Word report with complete analysis")
    ...
    with col3:
        ...
        try:
            ...
            st.download_button(label="⬇️ Download PDF Report", data=pdf_bytes, file_name="skill_gap_analysis_report.pdf", mime="application/pdf", help="Download formatted PDF report with visualizations")
        except Exception as e:
    ...
    **📄 Multi-page Support:** This tool fully supports multi-page resumes and job descriptions in PDF, DOCX, and TXT formats.
    ...
    ### 🚀 New Features:
    - **Enhanced PDF extraction** - Better handling of multi-page PDFs
    - **DOCX table support** - Extracts skills from tables in Word documents
    - **Character/word counts** - See document statistics
    - **Progress indicators** - Know when long documents are being processed
    - **GitHub integration** - Automatically detect and analyze GitHub profiles
    - **Smart validation** - Clear error messages if files are missing or invalid
    """)