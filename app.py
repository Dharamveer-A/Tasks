...
# ---------------- PAGE CONFIG ----------------
st.set_page_config(
    page_title="Milestone 1 - Data Ingestion, Parsing and Cleaning",
    layout="wide"
)
...
if jd_file and resume_file :
    ...
    def calculate_skill_match(resume_skills, jd_skills):
        ...
        matched_pct = round((len(matched) / total) * 100, 2) if total else 0
        partial_pct = round((len(partial) / total) * 100, 2) if total else 0
        missing_pct = round((len(missing) / total) * 100, 2) if total else 0

        avg_pct = round((matched_pct + partial_pct) / 2, 2)
    ...
    def match_donut_chart(matched, partial, missing, title):
        sizes = [matched, partial, missing]
        labels = ["Matched", "Partially Matched", "Missing"]

        if sum(sizes) == 0:
            st.warning("No skills to visualize")
            return
    ...
    match_metrics = calculate_skill_match(
        set(resume_skills["technical"]) | set(resume_skills["soft"]),
        set(jd_skills["technical"]) | set(jd_skills["soft"])
    )
    ...
    missing_skills = match_counts["missing"]
    ...
    c1.metric("Overall Match", "72%")
    c2.metric("Matched Skills", "6")
    c3.metric("Missing Skills", "4")
    ...
    for skill, score in zip(["Python", "Machine Learning", "SQL"], [92, 88, 65]):
        st.write(skill)
